...
entity lfsr is
  generic (
    g_size : integer                           := 8;
    g_seed : std_logic_vector(64 - 1 downto 0) := x"FFFFFFFFFFFFFFFF"
  );
  port (
    i_clk   : in std_logic;
    i_rst_n : in std_logic;
    o_data  : out std_logic;
    o_state : out std_logic_vector(g_size - 1 downto 0)
  );
end entity lfsr;
architecture rtl of lfsr is
  ...
  signal r_state : std_logic_vector(g_size - 1 downto 0) := g_seed(g_size - 1 downto 0);
begin
  -- check g_size
  ...
  MAIN : process (i_clk) is begin
    if rising_edge(i_clk) then
      if i_rst_n = '0' then
        r_state <= g_seed(g_size - 1 downto 0);
      else
